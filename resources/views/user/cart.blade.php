                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @foreach ($products as $product)
                        <div class="mt-4 flex gap-2 items-center">
                            ...
                            <div class="w-2/12">{{ $product->pivot->quantity }}個</div>
                            <div class="w-4/12">{{ number_format($product->pivot->quantity * $product->price) }}円（税込）</div>
                            <form class="w-3/12" action="{{ route('user.cart.destroy', $product) }}" method="POST">
                                @csrf
                                @method('DELETE')
                    ...
                    <div>合計金額{{ number_format($totalPrice) }}円（税込）</div>
                    <form action="{{ route('user.cart.checkout') }}" method="post">
                        @csrf
                        <x-form.button>購入する</x-form.button>
                    </form>
